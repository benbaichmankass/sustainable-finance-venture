...
def main():
    ...
    claims = [
        ...
        ("docs/research/research-agenda.md",
         r"P1 components read \((\d+) of (\d+) complete",
         (len([r for r in lc if r["Priority"] == "P1" and r["Status"] == "Reviewed"]),
          tiers["P1"]["components"])),
        ("README.md",
        ...
        ("README.md",
         r"\| Literature anchors \| (\d+) logged \((\d+) reviewed, (\d+) to read\) . against a P1 target of ~(\d+)",
         (len(lit),
          len([r for r in lit if r["Status"] == "Reviewed"]),
          len([r for r in lit if r["Status"] == "To read"]),
          tiers["P1"]["target"])),
        ("README.md",
